...
class ErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean; error: Error | null }> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) { console.error("Terminal Crash Log:", error, errorInfo); }
  render() {
      ...
      return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-8">
          <div className="max-w-md w-full bg-white p-10 rounded-[32px] border border-slate-200 shadow-2xl space-y-6">
            ...
            <button onClick={() => window.location.reload()} className="w-full py-4 bg-slate-900 text-white rounded-xl font-bold uppercase tracking-widest text-[11px] hover:bg-slate-800 transition-all active:scale-95">Reload Terminal</button>
          </div>
        </div>
      );
    }
...
export default function App() {
  ...
  const [processedOpenId, setProcessedOpenId] = useState(false);
  ...
  useEffect(() => {
    if (!processedOpenId && vehicles.length > 0) {
      const params = new URLSearchParams(window.location.search);
      const openVehicleId = params.get('openVehicleId');
      if (openVehicleId) {
      ...
      setProcessedOpenId(true);
    }
  }, [vehicles, processedOpenId]);

  const handleLogin = (identifier: string, isAdmin: boolean) => {
    console.log("App handleLogin called: identifier=", identifier, "isAdmin=", isAdmin);
  ...
  return (
    <ErrorBoundary>
      <div className="h-screen w-screen bg-[#f3f4f9] flex font-sans overflow-hidden">
        {/* Sidebar */}
        {isSidebarOpen && (
          <aside className="w-72 bg-[#0a1128] flex-shrink-0 flex flex-col h-full text-white overflow-y-auto no-scrollbar no-print">
            {/* Sidebar Logo */}
        ...
        {/* View-specific container with conditional scrolling */}
            ...
            <AnimatePresence mode="wait">
               ...
               >
                ...
                {activeView === 'tax' && (
                  <TaxDashboard 
                    vehicles={vehicles}
                    onImport={() => setShowImport({ active: true, type: 'tax' })}
                    onAdd={() => setShowForm({ active: true, type: 'tax', data: null })}
                    onEdit={(t: any) => setShowForm({ active: true, type: 'tax', data: t })}
